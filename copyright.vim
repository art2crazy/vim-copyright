...
let s:file_copyright_comment_prefix = ""
let s:file_copyright_comment_mid_prefix = ""
let s:file_copyright_comment_end = ""
let s:file_copyright_preserve = ""
...
let g:file_copyright_preserve_map_default = {}

if !exists('g:file_copyright_preserve_map')
  let g:file_copyright_preserve_map = {}
endif
...
function SetCommentFlag()
  " default
  let s:file_copyright_comment_prefix = "\#"
  " search default hardcoded map only if default not set by user
  if !exists('g:file_copyright_comment_prefix')
    for item in keys(g:file_copyright_comment_prefix_map_default)
      if item == &filetype
        let s:file_copyright_comment_prefix = g:file_copyright_comment_prefix_map_default[&filetype]
      endif
    endfor
  else
    let s:file_copyright_comment_prefix = g:file_copyright_comment_prefix
  endif
  " if user set his/her own, overwrite defaults
  for item in keys(g:file_copyright_comment_prefix_map)
    if item == &filetype
      let s:file_copyright_comment_prefix = g:file_copyright_comment_prefix_map[&filetype]
    endif
  endfor
  ...
  if !exists('g:file_copyright_comment_mid_prefix')
    for item in keys(g:file_copyright_comment_mid_prefix_map_default)
      if item == &filetype
        let s:file_copyright_comment_mid_prefix = g:file_copyright_comment_mid_prefix_map_default[&filetype]
      endif
    endfor
  else
    let s:file_copyright_comment_mid_prefix = g:file_copyright_comment_mid_prefix
  endif
  for item in keys(g:file_copyright_comment_mid_prefix_map)
    if item == &filetype
      let s:file_copyright_comment_mid_prefix = g:file_copyright_comment_mid_prefix_map[&filetype]
    endif
  endfor
  ...
  if !exists('g:file_copyright_comment_end')
    for item in keys(g:file_copyright_comment_end_map_default)
      if item == &filetype
        let s:file_copyright_comment_end = g:file_copyright_comment_end_map_default[&filetype]
      endif
    endfor
  else
    let s:file_copyright_comment_end = g:file_copyright_comment_end
  endif
  for item in keys(g:file_copyright_comment_end_map)
    if item == &filetype
      let s:file_copyright_comment_end = g:file_copyright_comment_end_map[&filetype]
    endif
  endfor

  let s:file_copyright_preserve = ""
  if !exists('g:file_copyright_preserve')
    for item in keys(g:file_copyright_preserve_map_default)
      if item == &filetype
        let s:file_copyright_preserve = g:file_copyright_preserve_map_default[&filetype]." "
      endif
    endfor
  else
    let s:file_copyright_preserve = g:file_copyright_preserve." "
  endif
  for item in keys(g:file_copyright_preserve_map)
    if item == &filetype
      let s:file_copyright_preserve = g:file_copyright_preserve_map[&filetype]." "
    endif
  endfor
endfunction
...
function! <SID>SetComment(begin)
    ...
    let l += 1
    call append(l,       s:file_copyright_comment_prefix)
    let l += 1
    call append(l,   s:file_copyright_comment_mid_prefix.s:file_copyright_preserve."Copyright (C) ".strftime("%Y")." ".expand(g:file_copyright_company)." ".expand(g:file_copyright_rights))
    let l += 1
    call append(l,   s:file_copyright_comment_mid_prefix."Unauthorized copying of this file, via any medium is strictly prohibited")
    let l += 1
    call append(l,   s:file_copyright_comment_mid_prefix."Proprietary and confidential")
    let l += 1
    call append(l,   s:file_copyright_comment_mid_prefix)
    let l += 1
    call append(l,   s:file_copyright_comment_mid_prefix."Author        : ".expand(g:file_copyright_name))
    let l += 1
    call append(l,   s:file_copyright_comment_mid_prefix."Email         : ".expand(g:file_copyright_email))
    let l += 1
    call append(l,   s:file_copyright_comment_mid_prefix."Created       : ".strftime("%Y-%m-%d %H:%M"))
    let l += 1
    call append(l,   s:file_copyright_comment_mid_prefix."Last Modified : ".strftime("%Y-%m-%d %H:%M"))
    let l += 1
    call append(l,  s:file_copyright_comment_end)
    let l += 1
    call append(l, "")
...
function! <SID>UpdateFileHead(add)
    ...
    call SetCommentFlag()
    let n = 1
    let regline = '^.*'.s:file_copyright_comment_mid_prefix.'.*Last\sModified\s:\s*\S*.*$'
    while n < 15
        let line = getline(n)
        if line =~ regline
...
function! <SID>UpdateTitle()
    silent! execute '/^.*'.s:file_copyright_comment_mid_prefix.'.*Last\sModified\s:/s@:.*$@\=strftime(": %Y-%m-%d %H:%M")@'
"    execute '/^'.s:file_copyright_comment_mid_prefix.'File\sName\s*:/s@:.*$@\=": ".expand("%:t")@'
    silent! execute "noh"
